...
int BinarySearchOccurances(int arr[], int arr_size, int target, int searchFirst){
	...
	while(end >= start){
		mid = start + (end - start)/2;
				switch (searchFirst){
				case 0: // last occur
					//only choose the target that the element after it is higher.
					if((target == arr[mid]) && ((target < arr[mid+1]) || (mid == arr_size - 1))){
						return mid;
					}
					...
					break;
				case 1:	//first occur
					//only choose the target that the element before it is lower.
					if((target == arr[mid]) && ((target > arr[mid-1]) || (mid == 0))){
						return mid;
					}
					else if(target>arr[mid]){
						start = mid +1 ;
